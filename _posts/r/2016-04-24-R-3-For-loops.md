---
layout: post
title: Learning R <br> 3. For loops and random walks
author: matt_sosna
---
...
## Footnotes
1. [[For loops]](#for-loops) When you iterate through an index instead of the values themselves, you can more easily update other variables with the results of the loop.
```r
result_vec <- c()
...
2. [[Random walks]](#random-walks) Note that we could set `mean_step` equal to some positive or negative value; that just means that our walk would drift upward or downward. By setting it to zero, our walk is unbiased: at each iteration, it's equally likely to take a step downward as it is to take a step upward.
