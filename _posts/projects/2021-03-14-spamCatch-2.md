...
- [Finishing up Flask]({{  site.baseurl  }}/spamCatch-3/#finishing-up-flask)
...
In line 24 we manually specify that our target class is `spam`. (Pro tip for avoiding mysterious bugs where your model is actually predicting *the opposite* class!) And in lines 26 onward, we split our data into training and testing, fit our classifier, and then update the `model`, `accuracy`, and `top_features` attributes of our class.
