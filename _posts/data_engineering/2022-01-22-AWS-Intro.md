...
## What is the cloud?
To cut to the chase, the cloud is just a whole bunch of computers sitting in **data centers**. A _data center_ is a fancy, secure warehouse for storing a _lot_ of running computers, likely located somewhere where the electricity is cheap [or the climate is cold](https://www.facebook.com/zuck/posts/10103136694875121).

These computers don't have monitors or keyboards $-$ they're just the hardware that performs calculations, stores and retrieves data from memory, responds to [HTTP requests](https://developer.mozilla.org/en-US/docs/Web/HTTP), etc. We call these machines **servers** to distinguish them from the human-friendly laptops or desktops most people are familiar with, as they _serve_ responses to user requests.
...
The data centers of the largest tech companies $-$ Amazon, Meta (Facebook), Google, etc. $-$ have _millions_ of servers stacked on top of each other in rack after rack extending [as far as you can see](https://www.google.com/about/datacenters/podcast/). Any time a person uploads a cat photo to Instagram, or bookmarks a pair of shoes on Pinterest, or responds to a message on WhatsApp, they interact with a few of these servers.<sup>[[1]](#1-what-is-the-cloud)</sup>  

By distributing the workload across these millions of machines, they're able to do XYZ.
...
Technically, your HTTP request to view Justin Bieber's latest Tweet likely doesn't make it all the way to one of these massive data centers. [Content delivery networks (CDNs)](https://en.wikipedia.org/wiki/Content_delivery_network) alleviate the load on data centers (and the internet as a whole) by [caching](https://www.cloudflare.com/learning/cdn/what-is-caching/) popular content. Data can be retrieved much faster from a CDN than a database in a data center because the requested content is stored in memory, meaning the server doesn't have to search through billions or trillions of pieces of content to find what was requested. By scattering hundreds or thousands of these mini data centers around the world $-$ especially in densely-populated areas $-$ many requests can be fulfilled much faster. This is indeed the main way [Netflix can offer such a seamless streaming experience](https://blog.apnic.net/2018/06/20/netflix-content-distribution-through-open-connect/).

But of course, there are limitations to CNDs or they'd be all we use. Servers have a lot less memory available than storage. Objects in memory disappear if the server loses power even for a moment, unlike objects written to storage. (Think about how your internet tabs or unsaved documents can disappear if your computer turns off, or how a game you were playing on your phone has to start up again if your phone restarts.) For stuff like user data, it's also not necessary to have the data available lightning-fast: it can be more important to be able to efficiently store a lot of data.
