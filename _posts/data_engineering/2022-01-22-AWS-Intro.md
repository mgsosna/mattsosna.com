...
We made it! Should we start building an [image classifier](https://docs.aws.amazon.com/sagemaker/latest/dg/image-classification.html) or a [chatbot](https://aws.amazon.com/chatbot/)? How about a [NoSQL database](https://aws.amazon.com/dynamodb) to store scraped data from Craigslist? Actually... **let's start with identity management.**

It might sound strange to kick off our AWS series by talking about identity management, but following security best practices is a foundation for successfully using the cloud. Customers aren't very sympathetic if you get hacked, and it is _very_ easy to rack up thousands of dollars in

It's also easy to pull in new developers to your project if you start with good access management.


it's important to lock this down before we go any further. When dealing with cloud resources, you're referencing APIs that are open to the world. When an AWS server gets a request to open a file or run some code, how does it know that that request came from you and not some random intruder?
