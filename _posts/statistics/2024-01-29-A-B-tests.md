---
layout: post
title: Navigating Uncertainty with Experiments
author: matt_sosna
tags: statistics
---

**"What if?"** Every decision we make is a bet that the path we choose is better than the others. When the paths don't differ much, the choice is usually fast and quickly forgotten. But when getting the path right really matters -- when there are serious consequences to making a mistake -- what can guide us to the right choice?

Imagine, for example, that you're running an e-commerce startup and just launched a "Products You Might Like" service. Your engineers invested a lot of time building what seemed like a great [recommender system](https://www.nvidia.com/en-us/glossary/recommendation-system/), but customer engagement is way lower than you expected. Your company is burning through cash, so getting these recommendations right is important. **But what if changing the algorithm actually _worsens_ engagement?** (More existentially, what if the added revenue from a PYML service never outweighs the cost of running it?) In this high-pressure moment, how do you decide between staying the course or pivoting?
