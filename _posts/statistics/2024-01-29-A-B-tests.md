---
layout: post
title: Navigating Big Decisions with Experimentation
author: matt_sosna
tags: statistics
---

**"What if?"** Every decision we make is a bet that the path we choose will lead us to our goal. When it's obvious which path to take, the choice is fast and quickly forgotten. But when the paths are hard to distinguish -- and especially when there are serious consequences to making a mistake -- what can guide us to the right choice?

Imagine, for example, that you're running an e-commerce startup and just launched a "Products You Might Like" service. Your engineers invested a lot of time building what seems like a great [recommender system](https://www.nvidia.com/en-us/glossary/recommendation-system/), but customer engagement is far lower than you expected. Your company is burning through cash, so getting these recommendations right is important. **But what if changing the algorithm actually _worsens_ engagement?** (More existentially, what if the added revenue from a PYML service never outweighs the cost of running it?) In this high-pressure moment, how do you decide between staying the course or pivoting?
